# Écosystème de Partenaires

## Objectif

Définir la stratégie de partenaires nécessaire pour développer, financer, construire et exploiter des solutions de flexibilité industrielle à grande échelle.

L'objectif n'est pas de tout faire.

L'objectif est de déterminer :

- ce qu'COMMERCIAL doit maîtriser
- ce qu'COMMERCIAL doit orchestrer
- ce qu'COMMERCIAL peut externaliser
- ce qui crée un avantage compétitif durable

## Principe fondamental

Un projet de flexibilité est rarement réalisé par une seule entreprise.

La création de valeur dépend de la qualité de l'écosystème.

Le rôle d'COMMERCIAL est d'orchestrer cet écosystème autour du client.

## Vision

Le client ne recherche pas :

- une batterie
- un EMS
- un agrégateur
- un contrat

Le client recherche une solution.

Le client attend qu'COMMERCIAL coordonne les acteurs nécessaires.

## Catégories de partenaires

### OEM Batteries

Rôle :

- cellules
- modules
- racks
- garanties
- performance
- support technique

Critères de sélection :

- bancabilité
- historique
- garanties
- disponibilité
- roadmap produit
- présence européenne

Questions clés :

- Qui porte la garantie ?
- Quelle capacité est garantie ?
- Quel throughput est autorisé ?
- Quelle dégradation est couverte ?

## Fournisseurs PCS

Rôle :

- conversion d'énergie
- contrôle de puissance
- intégration réseau

Critères :

- performance
- fiabilité
- conformité réseau
- disponibilité

## Fournisseurs EMS

Rôle :

- optimisation énergétique
- pilotage
- contrôle
- reporting
- data

Critères :

- cybersécurité
- interopérabilité
- évolutivité
- capacité multi-actifs

Question stratégique :

COMMERCIAL doit-il utiliser des EMS tiers ou développer ses propres capacités ?

## EPC

Rôle :

- ingénierie
- construction
- mise en service

Critères :

- expérience BESS
- sécurité
- exécution
- qualité
- couverture géographique

Risques :

- retard
- surcoût
- performance

## Intégrateurs

Rôle :

- intégration des différents composants
- tests
- mise en service

Valeur :

Réduction de la complexité d'intégration.

## Agrégateurs

Rôle :

- accès aux marchés
- agrégation
- activation
- settlement

Critères :

- performance
- transparence
- couverture géographique
- portefeuille existant

Questions clés :

- Qui contrôle le dispatch ?
- Qui porte le risque marché ?
- Qui conserve l'upside ?

## Optimisateurs

Rôle :

- forecast
- bidding
- trading
- optimisation multi-marchés

Valeur :

Transformer la flexibilité en revenus.

Questions clés :

- Comment la performance est-elle mesurée ?
- Comment l'upside est-il partagé ?
- Quels KPI sont utilisés ?

## Fournisseurs de recharge

Rôle :

- bornes
- smart charging
- V2G

Critères :

- interopérabilité
- pilotabilité
- ouverture des données

## Fournisseurs de photovoltaïque

Rôle :

- production locale
- autoconsommation

Valeur :

Création de flexibilité supplémentaire via couplage avec BESS.

## Investisseurs

Rôle :

- financement
- ownership

Types :

- infrastructure funds
- pension funds
- asset managers
- project finance lenders

Questions clés :

- Quel rendement recherchent-ils ?
- Quel niveau de risque acceptent-ils ?
- Quelle visibilité exigent-ils ?

## Assureurs

Rôle :

- protection
- transfert de risque

Domaines :

- construction
- exploitation
- cybersécurité
- responsabilité civile

## Partenaires digitaux

Rôle :

- data
- IA
- forecasting
- monitoring

Valeur :

Amélioration de la qualité des décisions.

## Universités et centres de recherche

Rôle :

- innovation
- validation
- expertise

Utilisation :

- nouvelles technologies
- modèles de flexibilité
- marchés émergents

## Ce qu'COMMERCIAL doit conserver

Les compétences critiques à conserver peuvent inclure :

- relation client
- compréhension industrielle
- structuration commerciale
- business model
- risk allocation
- structuration contractuelle
- financement
- gouvernance
- orchestration des partenaires

Ces compétences créent la différenciation.

## Ce qu'COMMERCIAL peut externaliser

Selon les cas :

- fabrication
- EPC
- maintenance
- certains logiciels
- certaines fonctions d'agrégation

L'externalisation ne doit pas créer une dépendance excessive.

## Critères de sélection des partenaires

### Technique

- qualité
- performance
- expérience
- références

### Commercial

- compétitivité
- flexibilité
- capacité d'adaptation

### Financier

- solidité
- pérennité
- capacité d'investissement

### Stratégique

- compatibilité avec la vision COMMERCIAL
- potentiel européen
- capacité d'innovation

## Stratégie européenne

COMMERCIAL doit maximiser :

- standardisation
- réplicabilité
- rapidité de déploiement

Le portefeuille de partenaires doit être :

- européen
- multi-pays
- résilient
- évolutif

## Risques liés aux partenaires

Risques possibles :

- dépendance excessive
- concentration
- faillite
- performance insuffisante
- retard
- incompatibilité technique
- perte de propriété intellectuelle

## Questions à poser à chaque partenaire

- Quelle valeur apportez-vous ?
- Quel risque prenez-vous ?
- Quel risque transférez-vous ?
- Quelles garanties fournissez-vous ?
- Comment mesurez-vous la performance ?
- Comment êtes-vous rémunéré ?

## Vision cible

COMMERCIAL agit comme orchestrateur.

Les partenaires apportent :

- technologies
- financement
- exécution
- optimisation

COMMERCIAL assemble ces capacités pour créer une offre de flexibilité industrielle cohérente, réplicable et rentable.

## Conclusion

La valeur ne réside pas dans un partenaire individuel.

La valeur réside dans la capacité à construire, gouverner et faire évoluer un écosystème capable de résoudre les problèmes industriels des clients à grande échelle.
